...
### `/request`
Запрос на создание брони, либо запроса по остаткам в торговой точке.
...
При отправке запроса, указываются два параметра:
* `X-Api-Version` - указывает версию протокола, с которой будет работать API.
Например: `X-Api-Version: 1.0`
* `X-Request-Type` - тип запроса, имеет два состояния: `booking`- для бронирования и `stock` - для запроса по остаткам. Например: `X-Api-Version: stock`
...
Пример команды при помощи служебной программы [cURL]:
```sh
curl -X POST -T "order.json" http://{addr}/booking/cli/id_get
```
Где `order.json` - набор данных JSON-формата, может быть отправлен в любом виде.
