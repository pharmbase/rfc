...
6. Для выполнения каждого из нижеприведенных методов используется `отдельное` соединение, которое после завершения работы в штатном режиме должно быть закрыто посредством команды `connect.Close()` из пакета [NATS].
...
Метод позволяющий получить информацию по товарам в конкретной торговой точке. Предусмотрен для взаимодействия торговых точек между собой. При указании соответствующего Юзер-Агента возможен обмен данными внутренними кодами торговой сети.
