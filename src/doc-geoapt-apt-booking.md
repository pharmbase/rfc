...
4. Для использования любых видов транспорта, кроме забора заказов по оффлайну, клиенту необходимо указать для необходимых идентификаторов торговых точек такие параметры как способ доставки, и адрес доставки. Если вид транспорта не указывается, по умолчанию используется транспорт оффлайн доставки `/pop_order`. Методы для каждого вида транспорта отдельные и указаны ниже.
...
При составлении названия, также стоит учитывать, что любые символы кроме цифр и латиницы - запрещены. Допускается символ `_` а также верхний регистр латиницы.

Пример команды при помощи служебной программы [cURL]:
```sh
curl -X POST -T "order.json" -u login:password http://{addr}/apt/reg_nats
```
Где: 
...
[
 {
   ...
   "msg_addr": "string"  // Название канала для данного идентификатора торговой точки     
 }
] 
```
...
[
 {
   "id_shop":  "556546", 
   "msg_addr": "hello_nats_1"      
 },
 {
   "id_shop":  "122254", 
   "msg_addr": "hello_nats_2"   
 }
] 
```
...
Пример команды при помощи служебной программы [cURL]:
```sh
curl -X POST -T "order.json" -u login:password http://{addr}/cli/get_id
```
Где: 
- `order.json` - набор данных JSON-формата.
- `{addr}`     - адрес API, который вы можете узнать у вашего менеджера.
...
Пример команды при помощи служебной программы [cURL]:
```sh
curl -X POST -T "order.json" -u login:password http://{addr}/apt/pop_order
```
Где: 
- `order.json` - набор данных JSON-формата.
- `{addr}`     - адрес API, который вы можете узнать у вашего менеджера.
...
Пример команды при помощи служебной программы [cURL]:
```sh
curl -X POST -T "order.json" -u login:password http://{addr}/apt/upd_order
```
Где: 
- `order.json` - набор данных JSON-формата.
- `{addr}`     - адрес API, который вы можете узнать у вашего менеджера.
