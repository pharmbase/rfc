...
### Обмен данными
1. При использовании [NATS] соеденения, АПИ ожидает ответа от клиентов в момент запроса, в случае, если ответ не получен, запрос считается неудачным.
...
### `/get_order`
[HTTP] метод. Запрос на получение всех заказов для сети, которая учавствует в бронировании.
...
Допускается вариант, при котором торговая сеть может получить все заказы для неё, в таком случае, в запросе отправляется пустой массив.
Например:
```
[]
```
