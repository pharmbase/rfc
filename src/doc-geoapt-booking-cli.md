...
```
{
  "id": ["string"] // Идентификатор торговой точки (или несколько)
}
```
...
```
{
  ...
  "ver": "string",     // Версия АПИ, которую использует торговая точка
  "method": ["string"] // Перечень методов, которые использует торговая точка
}
```
...
В случае, если поле `state` содержит `REPEAT`, клиент может сделать повторный запрос, с обновленными данными, полученными от торговой точки:
```
{
    "meta": {
        "agent":   "string", // Маркер точки запроса
