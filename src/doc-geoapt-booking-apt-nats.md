...
Для этого, посредством драйвера [NATS] отправляются данные в канал `655c445.register`, который, после успешной проверки выдаст название двух каналов для работы с ними. 
...
3. При получении данных из прослушиваемого канала, торговая точка должна *присвоить* заказу один из двух статусов, в специальном поле формата, и отдать ответ по принципу Request–Response.

4. В случае отмена брони, ее продаже, или изменении номенклатуры при продаже, торговая точка *отсылает полный набор обновленных данных* во второй канал, который не прослушивается в реальном времени, по принципу Request–Response.
...
```
Параметр *X-Morion-Skynet-Key* вы можете узнать у вашего менеджера.
